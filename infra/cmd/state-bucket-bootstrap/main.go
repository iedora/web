...
const (
	...
	bucketLocation = "EEUR" // matches infra/tofu/variables.tf::data_bucket_location
)
...
func runLive(ctx context.Context, cfg config) error {
	...
	fmt.Fprintln(stderr, "  next: add the `backend \"s3\"` block to infra/tofu/versions.tf, then `tofu init -migrate-state`")
	return nil
}
