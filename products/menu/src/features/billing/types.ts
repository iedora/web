import type { InvoiceStatus } from '@iedora/billing'
 ...
 * (managed by `@iedora/billing`) so we can evolve the schema without
 * leaking columns into the UI.
 *
 * `plan` is menu's `PlanCode` (`'free'` / `'casa'`); we cast on the
 * way out of `@iedora/billing` (which stores it as an opaque string
