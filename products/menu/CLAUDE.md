...
apps/web/
  ...
  Dockerfile                         app build (Bun-install + Node-build + standalone). Same Dockerfile dev (built locally by `./bin/dev-stack`) and prod (built + pushed to Gitea OCI registry pelo job `deploy` de .gitea/workflows/ci.yml) consume.
