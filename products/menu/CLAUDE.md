...
apps/web/
  ...
  Dockerfile                         app build (Bun-install + Node-build + standalone). Same Dockerfile dev (built locally by `./bin/dev-stack`) and prod (built + pushed to Gitea OCI registry by .gitea/workflows/deploy.yml) consume.
  .env                               Auto-generated by ./bin/dev-stack on every run.
                                     Gitignored. Holds local stack defaults
                                     (infra-postgres, infra-s3mock infra-o2).
...
Dev: `./bin/dev-stack` boots the full local stack. See [docs/dev.md](../../docs/dev.md) for flags, HMR, and env file setup.

Prod: `.gitea/workflows/deploy.yml` corre `kamal deploy`, que faz build
remoto no Beelink, push para Gitea OCI registry, corre migrations via
pre-deploy hook, e faz blue-green swap.
...
- `./bin/dev-stack` (from repo root) — boots the local stack. See [docs/dev.md](../../docs/dev.md).
