...
apps/web/
  ...
  Dockerfile                         app build (Bun-install + Node-build + standalone). Same Dockerfile dev (built locally by `./bin/dev-stack`) and prod (built + pushed to GHCR by .github/workflows/web.yml) consume.
